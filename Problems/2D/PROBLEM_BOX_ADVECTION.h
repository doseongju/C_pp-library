...
public:
	static void Initialize(const int& resolution_x,
		...
		FIELD_UNIFORM_2D<double>& Q, double& t_final)
	{
		...
		/*grid.Initialize   (0, 0, resolution_x, resolution_x, x_lower, y_lower, x_upper, y_upper);
		const double dx = grid_xy.dx;
		const double dy = grid_xy.dy;
		grid_x.Initialize (0, 0, resolution_x + 1, resolution_x    , x_lower - 0.5*dx, y_lower         , x_upper + 0.5*dx, y_upper         );
		grid_y.Initialize (0, 0, resolution_x    , resolution_x + 1, x_lower         , y_lower - 0.5*dy, x_upper         , y_upper + 0.5*dy);
		grid_xy.Initialize(0, 0, resolution_x + 1, resolution_x + 1, x_lower - 0.5*dx, y_lower - 0.5*dy, x_upper + 0.5*dx, y_upper + 0.5*dy);*/

		grid_xy.Initialize(0, 0, resolution_x + 1, resolution_x + 1, x_lower, y_lower, x_upper, y_upper);
		const double dx = grid_xy.dx;
		const double dy = grid_xy.dy;
		grid_x.Initialize(0, 0, resolution_x + 1, resolution_x, x_lower, y_lower + 0.5*dy, x_upper, y_upper - 0.5*dy);
		grid_y.Initialize(0, 0, resolution_x, resolution_x + 1, x_lower + 0.5*dx, y_lower, x_upper - 0.5*dx, y_upper);
		grid.Initialize(0, 0, resolution_x, resolution_x, x_lower + 0.5*dx, y_lower + 0.5*dy, x_upper - 0.5*dx, x_upper - 0.5*dy);
		...
		ITERATION_GRID_2D(grid, 3)
		{
			...
			Q(i, j) = exp(-50 * SQR(x-0.5) - 50 * SQR(y-0.5));
			//Q(i, j) = 3*sin(r) / (r+1e-16);
		}
	}
};
