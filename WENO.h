...
public:
	WENO_EULER_SOLVER_1D(const double& gamma_):gamma(gamma_)
	{
		R_Q.Initialize(-3, 6);
		R_fu.Initialize(-3, 6);
		fu.Initialize(-3, 6);
...
public:
				...
				const double& alpha, VECTOR_3D<double>& flux)
	{
		...
		for(int p = 0; p < 3; p ++)
		{
			weno.ReconstructRight(fu_plus(-3)[p], fu_plus(-2)[p], fu_plus(-1)[p], fu_plus(0)[p], fu_plus(1)[p], left_r[p]);
			weno.ReconstructLeft(fu_minus(-2)[p], fu_minus(-1)[p], fu_minus(0)[p], fu_minus(1)[p], fu_minus(2)[p], right_l[p]);
			/*MUSCL<double>::Interapolation(fu_plus(-3)[p], fu_plus(-2)[p], fu_plus(-1)[p], left_r[p], right_r[p], fu_plus(0)[p], fu_plus(1)[p], fu_plus(2)[p], FIFTH_LIMITER, 2, 2);
			MUSCL<double>::Interapolation(fu_minus(-3)[p], fu_minus(-2)[p], fu_minus(-1)[p], left_l[p], right_l[p], fu_minus(0)[p], fu_minus(1)[p], fu_minus(2)[p], FIFTH_LIMITER, 2, 2);*/
			
		}
		flux  = (left_r[0]+right_l[0]) * r1 + (left_r[1]+right_l[1]) * r2 + (left_r[2]+right_l[2]) * r3;
	}

	void ComputedF(const VECTOR_3D<double>& lll, const VECTOR_3D<double>& ll, const VECTOR_3D<double>& l,
		const VECTOR_3D<double>& c, const VECTOR_3D<double>& r, const VECTOR_3D<double>& rr, const VECTOR_3D<double>& rrr,
		const double& alpha, VECTOR_3D<double>& dF)
	{
		VECTOR_3D<double> F_l, F_r;
		ComputeFlux(lll, ll, l, c, r, rr, alpha, F_l);
		ComputeFlux(ll, l, c, r, rr, rrr, alpha, F_r);
		dF = (F_r - F_l); //TODO : divide by dx.
	}
	
};

class WENO_EULER_SOLVER_2D
{
public:
	WENO5 weno;
...
public:
	WENO_EULER_SOLVER_2D(const double& gamma_ = 1.4):gamma(gamma_)
	{
		fu.Initialize(-3,6);
		gu.Initialize(-3,6);
...
public:
					  ...
					  const double& alpha, VECTOR_4D<double>& flux)
	{
		...
		for(int p = 0; p < 4; p ++)
		{
			weno.ReconstructRight(R_fu_plus(-3)[p], R_fu_plus(-2)[p], R_fu_plus(-1)[p], R_fu_plus(0)[p], R_fu_plus(1)[p], left_r[p]);
			weno.ReconstructLeft(R_fu_minus(-2)[p], R_fu_minus(-1)[p], R_fu_minus(0)[p], R_fu_minus(1)[p], R_fu_minus(2)[p], right_l[p]);

			/*mlp.ReconstructRight(R_fu_plus (-3)[p], R_fu_plus (-2)[p], R_fu_plus(-1)[p], R_fu_plus (0)[p], R_fu_plus (1)[p], 2, left_r[p]);
			mlp.ReconstructLeft (R_fu_minus(-2)[p], R_fu_minus(-1)[p], R_fu_minus(0)[p], R_fu_minus(1)[p], R_fu_minus(2)[p], 2, right_l[p]);*/
		}
		...
		const double& alpha, VECTOR_4D<double>& flux)
	{
		...
		//double alpha_b, alpha_t;
		for(int p = 0; p < 4; p ++)
		{
			weno.ReconstructRight(R_gu_plus (-3)[p], R_gu_plus (-2)[p], R_gu_plus(-1)[p], R_gu_plus (0)[p], R_gu_plus (1)[p], bottom_t[p]);
			weno.ReconstructLeft (R_gu_minus(-2)[p], R_gu_minus(-1)[p], R_gu_minus(0)[p], R_gu_minus(1)[p], R_gu_minus(2)[p], top_b[p]   );
			
			/*mlp.ReconstructRight(R_gu_plus (-3)[p], R_gu_plus (-2)[p], R_gu_plus(-1)[p], R_gu_plus (0)[p], R_gu_plus (1)[p], 2, bottom_t[p]);
			mlp.ReconstructLeft (R_gu_minus(-2)[p], R_gu_minus(-1)[p], R_gu_minus(0)[p], R_gu_minus(1)[p], R_gu_minus(2)[p], 2, top_b[p]   );*/
