...
void main()
{
	...
	const int num_lev = 7;
	const int Nx_coarse = 65;
	const int Ny_coarse = 17;
	...
	DOUBLEMACH::Initialize(resolution, grid, grid_x, grid_y, grid_xy, Q, t_final, gamma);

	EULER_EQUATION_2D euler_eqn(gamma);
	FIELD_UNIFORM_2D<VECTOR_4D<double>> Q_0(grid, 3), Q_1(grid, 3), Q_2(grid, 3);
	ARRAY_1D<double> alpha_x(0, resolution), alpha_y(0,4*(resolution-1)+1);
	grid.PrintGridX("x");
	grid.PrintGridY("y");
	Q.TagName("Q");
	...
	BOUNDARY_MANAGER_DMACH boundary_manager_0(Q_0);
	BOUNDARY_MANAGER_DMACH boundary_manager_1(Q_1);
	BOUNDARY_MANAGER_DMACH boundary_manager_2(Q_2);
...
#pragma omp for
		...
		boundary_manager.SetBoundaryCondition(t);
...
#pragma omp for
		...
		boundary_manager_2.SetBoundaryCondition(t);
		alpha_x.AssignAllValue(0), alpha_y.AssignAllValue(0);
...
#pragma omp for
		...
		boundary_manager_1.SetBoundaryCondition(t);
		alpha_x.AssignAllValue(0), alpha_y.AssignAllValue(0);
